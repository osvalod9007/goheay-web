...
import {Col, Icon, Modal, Table, Tooltip, Pagination, Popconfirm, Row} from 'antd';
...
import './VehicleTypesListTable.less';
...
const VehicleTypesListTable = observer((props: any) => {
  ...
  const convertOrder = [{key: 'type', value: 'type'}, {key: 'created', value: 'createdAt'}];
  ...
  const deleteElem = async (id: any) => {
    try {
      ...
      const itemToDelete: any = await vehicleTypesModel.delete({where: [{field: 'id', value: id, isPk: true}]});
      if (itemToDelete && itemToDelete.data.VehicleTypeDelete.isSuccess) {
        const msg = `${t('labelA')} ${t('labelVehicleType')} ${t('labelWasDeleted')}.`;
    ...
    const onCompleted = result => {
      ...
      const mappedDataTmp = items.map(item => {
        ...
        return {
          ...
          key: number,
          type: type || '',
          image: image && image.publicUrl ? image.publicUrl : '',
          dimensions:
            `${parseFloat(measureMinDimension.amount).toFixed(2)} - ${parseFloat(measureMaxDimension.amount).toFixed(
              2,
            )}` || '',
          payload:
            `${parseFloat(measureMinPayload.amount).toFixed(2)} - ${parseFloat(measureMaxPayload.amount).toFixed(2)}` ||
            '',
  ...
  const columns = [
    ...
    },
    {
      ...
      sorter: false,
      render: (item: any) => {
        return <ImagePreview imageUrl={item.image} width={'36px'} />;
      },
    },
    {
      title: t('labelDimensionsRange'),
      dataIndex: 'dimensions',
      width: '13%',
      sorter: false,
    },
    {
      title: t('labelPayloadRange'),
      dataIndex: 'payload',
      width: '11%',
      sorter: false,
    },
    ...
    },
    {
      ...
      render: item => (
        <span>
          {appStore.hasPermissions(appStore.permissions, ['VehicleTypeUpdate']) && (
            <React.Fragment>
              <RiftLink to={`/admin/vehicletype/${item.id}`} className="vehicle-types-list-table__pointer">
          ...
          {appStore.hasPermissions(appStore.permissions, ['VehicleTypeDelete']) && (
            <React.Fragment>
              <Tooltip title={t('labelDelete')}>
                <Popconfirm
                  icon={<Icon type="question-circle-o" style={{color: '#e33244'}} />}
                  placement="topRight"
                  title={`${t('msgConfirmDeleteElem')} ${t('labelVehicleType')}?`}
                ...
                >
                  <Icon
                    type="delete"
                    style={{fontSize: '18px'}}
                    className="vehicle-types-list-table__pointer"
                    theme="twoTone"
                    twoToneColor="#e33244"
                  />
                </Popconfirm>
  ...
  return (
    <div style={{overflow: 'hidden', overflowX: 'auto'}} className="vehicle-type-list-table">
      <div style={{minWidth: 1070}}>
        <Table
          ...
          size="middle"
          locale={{emptyText: <span>{`${t('msgThereAreNo')} ${t('labelVehicleTypes')} ${t('msgToDisplay')}.`}</span>}}
        />
