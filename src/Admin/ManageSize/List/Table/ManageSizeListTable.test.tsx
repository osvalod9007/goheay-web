...
import ManageSizeListTable from './ManageSizeListTable';
...
describe('Manage Size List Table Test', () => {
  const config = {
    pageSize: 5,
    page: 1,
    keyword: '',
  ...
  it('Manage Size Table renders correctly', () => {
    const client = createClient(HELLO_MOCKS);
    shallow(
      <ApolloProvider client={client}>
        <ManageSizeListTable input={config} updateItems={updateItems} />
      </ApolloProvider>,
    );
  });
});
